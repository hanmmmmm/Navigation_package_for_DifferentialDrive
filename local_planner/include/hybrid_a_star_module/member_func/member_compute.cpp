...
// int Hybrid_astar_class::check_collision(float x, float y, int scan_range)
// {
//     int xstart = std::max(int(x) - scan_range, 0);
//     int ystart = std::max(int(y) - scan_range, 0);
//     int xrange = std::min(scan_range * 2, fine_map_width_ - xstart - 1);
//     int yrange = std::min(scan_range * 2, fine_map_height_ - ystart - 1);
...
// void Hybrid_astar_class::map_inflation(cv::Mat *mapIn, cv::Mat *mapOut, int infla_size)
// {
...
// int Hybrid_astar_class::improve_hcost(std::array<float, 3> node_pose, int hcost, int scan_range, bool same_steer)
// {
